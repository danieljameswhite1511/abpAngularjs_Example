...
namespace abp_angularjs.Tests
{
    public abstract class abp_angularjsTestBase : AbpIntegratedTestBase<abp_angularjsTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected abp_angularjsTestBase()
        {
        ...
        protected void UsingDbContext(Action<abp_angularjsDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<abp_angularjsDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<abp_angularjsDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<abp_angularjsDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<abp_angularjsDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<abp_angularjsDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<abp_angularjsDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<abp_angularjsDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<abp_angularjsDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<abp_angularjsDbContext, Task<T>> func)
        {
            T result;
